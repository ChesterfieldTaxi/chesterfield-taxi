...
export class OperatorService {
  ...
  public getDriverRoster(): DriverRosterItem[] {
    ...
    const operatorDrivers: DriverRosterItem[] = driverOps.map((op) => {
      ...
      return {
        ...
        vehicle: op.assignedUnit || 'Unassigned',
        tier: 'Sedan',
        phone: op.phone || '',
  ...
  public async saveOperator(operator: OperatorUser): Promise<void> {
    if (isFirebaseConfigured()) {
      try {
        const db = getFirestore(getFirebaseApp());
        await setDoc(doc(db, 'users', operator.uid), operator, { merge: true });
      } catch (err) {
        console.warn('[OperatorService] Firestore save error:', err);
      }
    }

    const idx = this.operators.findIndex((o) => o.uid === operator.uid);
    if (idx >= 0) {
      this.operators[idx] = { ...this.operators[idx], ...operator };
    } else {
      this.operators = [operator, ...this.operators];
    }
